...
function robotDraw(startCode, input) {
  ...
  const image = new Array(100).fill(0).map((item) => new Array(100).fill(' '));
  const allPositions = Object.keys(memoryDraws);
  for (let i = 0; i < allPositions.length; i += 1) {
    const pos = allPositions[i];
    const color = memoryDraws[pos];
    const splitedPos = pos.split(',').map((item) => Number(item));
    if (color === 1) {
      image[50 + splitedPos[0]][50 + splitedPos[1]] = '❚';
    }
  }
  image.map(item => item.join('')).forEach(item => console.log(item));
}

robotDraw(1,['input here']);
