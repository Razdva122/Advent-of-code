function crossedWires(arrayOfMoves1, arrayOfMoves2) {
  const memory = {};
  const firstWireCoord = [0, 0];
  const secondWireCoord = [0, 0];
  const moves = {
    R: [1, 0],
    L: [-1, 0],
    U: [0, 1],
  ...
  const distance = [];
  let countSteps1 = 0;
  for (let i = 0; i < arrayOfMoves1.length; i += 1) {
    const currentMove = arrayOfMoves1[i];
    const direction = moves[currentMove[0]];
    const amountOfSteps = Number(currentMove.slice(1));
    for (let j = 1; j <= amountOfSteps; j += 1) {
      countSteps1 += 1;
      firstWireCoord[0] += direction[0];
      firstWireCoord[1] += direction[1];
      if (!memory[firstWireCoord.join()]) {
        memory[firstWireCoord.join()] = { isFirst: true, distance: countSteps1 };
      }
    }
  }

  let countSteps2 = 0;
  for (let i = 0; i < arrayOfMoves2.length; i += 1) {
    const currentMove = arrayOfMoves2[i];
    const direction = moves[currentMove[0]];
    const amountOfSteps = Number(currentMove.slice(1));
    for (let j = 1; j <= amountOfSteps; j += 1) {
      countSteps2 += 1;
      secondWireCoord[0] += direction[0];
      secondWireCoord[1] += direction[1];
      const addressMemory = memory[secondWireCoord.join()];
      if (addressMemory && addressMemory.isFirst) {
        addressMemory.isFirst = false;
        distance.push(addressMemory.distance + countSteps2);
      }
    }
  }
