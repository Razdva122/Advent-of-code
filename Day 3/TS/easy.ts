function crossedWires(arrayOfMoves1: Array<string>, arrayOfMoves2: Array<string>) :number {
  interface Memory {
    [key: string]: boolean;
  }
  const memory: Memory = {};
  ...
  for (let i = 0; i < arrayOfMoves1.length; i += 1) {
    const currentMove: Array<string> = arrayOfMoves1[i].split('');
    const direction: string = currentMove[0];
    const coords: [number, number] = moves[direction];
    const amountOfSteps: number = Number(currentMove.slice(1));
  ...
  for (let i = 0; i < arrayOfMoves2.length; i += 1) {
    const currentMove: Array<string> = arrayOfMoves2[i].split('');
    const direction: string = currentMove[0];
    const coords: [number, number] = moves[direction];
