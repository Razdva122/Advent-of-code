...
function createMap(rawData) {
  ...
  image.forEach((row, rowIndex) => {
    row.forEach((cell, cellIndex) => {
      image[rowIndex][cellIndex] = String.fromCharCode(cell);
    });
  });
  return image;
}
...
function calibrate(image) {
  const intersections = [];
  const closeCells = [[0, 1], [0, -1], [1, 0], [-1, 0]];
  image.forEach((row, rowIndex) => {
    row.forEach((cell, cellIndex) => {
      if (cell === '#') {
        const isIntersection = closeCells.every((coords) => {
          return image[coords[0] + rowIndex][coords[1] + cellIndex] === '#';
        });
        if (isIntersection) {
          intersections.push([rowIndex, cellIndex]);
        }
      }
    });
  });
  const sumOfAlign = intersections.reduce((acc,coords) => {
